...
'''
#将老师所在4列数据合并写在AE（31）列
#遍历第二行-最后一行
for rowNum in range(2, sheet2.max_row+1):
    #设置每一行初始老师名称j为空
    j=str()
    #遍历第8列-11列，并把每个单元格的文本合并到j上
    for columnNum in range (8,12):
        if sheet2.cell(row=rowNum,column=columnNum).value != None:
            j=str(j)+str(sheet2.cell(row=rowNum,column=columnNum).value)
    sheet2.cell(row=rowNum, column=31).value = j
        #sheet2.merge_cells("H"+str(rowNum)+":"+"K"+str(rowNum))失效
...
for rowNum in range(2, sheet2.max_row+1):
    ...
    #遍历第7列，并把每个单元格的数值加到i上
    #for columnNum in range (7):
    i=sheet2.cell(row=rowNum,column=31).value
    #将i写入所在行的第30列单元格
    #sheet2.cell(row=rowNum, column=30).value = i
    sheet3.cell(row=rowNum, column=2).value = i
...
wb5.save(str(dirname)+'\\'+str(newdir)+'\\'+'summary.xlsx')
'''
...
#填充课程和老师名称
    ...
    # lsmc = sheet2['I' + str(row)].value
    # lsmc = sheet2['J' + str(row)].value
    # lsmc = sheet2['K' + str(row)].value
    #填充到字典中
    jxpj.setdefault(kcmc, {})
    jxpj[kcmc].setdefault(lsmc, {})
    jxpj[kcmc][lsmc].setdefault(zf)
print(jxpj)
'''
print('All Done.\n')
